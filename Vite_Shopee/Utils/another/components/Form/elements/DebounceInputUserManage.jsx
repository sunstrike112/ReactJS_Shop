...
export function InputDebounceFormUserManage({ action, debounce = 500, label, haveIconSearch = false, ...props }) {
  const [suggestedValues, setSuggestedValues] = useState([]);
  const [selectedValue, setSelectedValue] = useState('');
  const [ref, isMouseOver] = useMouseOver();
  const inputRef = useRef(null);
  const getSessionValues = () => {
    const savedValues = sessionStorage.getItem('suggestedValueUserManage');
  ...
  const saveToSessionStorage = (value) => {
    ...
    sessionStorage.setItem('suggestedValueUserManage', JSON.stringify(savedValues));
  };
            ...
            >
              ...
              <Box
                onClick={(e) => {
                  ...
                  sessionStorage.setItem('suggestedValueUserManage', JSON.stringify(filteredValues));
