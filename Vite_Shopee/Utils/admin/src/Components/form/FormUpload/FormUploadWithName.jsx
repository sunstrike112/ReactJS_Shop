...
const FormUploadWithName = ({ t, label, name, rules, defaultValue = '', wrapperProps, size = FILE_SIZE, sizeRequired = '10240 KB', acceptFormat = ACCEPT_FORMAT, ...rest }) => {
      ...
      if (acceptFormat !== '*' && !FILE_FORMATS.includes(file.type)) {
        ...
        setError(name, {
          ...
          message: trans('error_message:validation.max_file_size',
            { fileName: file.name, size: file.size / 1024, sizeRequired })
        })
      } else {
  ...
  return (
    ...
    >
      <Upload
        ...
        fileList={files}
        itemRender={(_, uploadFileRender, c, d) => {
          return (
            <WrapperContent align="middle" justify="center" status={uploadFileRender.status}>
              {uploadFileRender.status === 'uploading' && (
                ...
                <>
                  <Row className="upload-content" align="middle" justify="center">
                    <Col span={24} className="upload-content-col">
                      {FILE_AUDIO_FORMATS.includes(uploadFileRender.type) && (
                        <audio src={uploadFileRender.response || uploadFileRender.thumbUrl} controls controlsList="nofullscreen noremoteplayback noplaybackrate nocaptions nodownload" />
                      )}
                      {FILE_IMG_FORMATS.includes(uploadFileRender.type) && (
                        (uploadFileRender.response || uploadFileRender.thumbUrl) && <img alt="example" src={uploadFileRender.response || uploadFileRender.thumbUrl} />
                      )}
                      {FILE_VIDEO_FORMATS.includes(uploadFileRender.type) && (
                        <video src={uploadFileRender.response || uploadFileRender.thumbUrl} controls controlsList="noremoteplayback noplaybackrate nocaptions nodownload" disablePictureInPicture />
                      )}
                      {(!FILE_AUDIO_FORMATS.includes(uploadFileRender.type)
                        && !FILE_IMG_FORMATS.includes(uploadFileRender.type)
                        && !FILE_VIDEO_FORMATS.includes(uploadFileRender.type))
                  ...
                  <Row align="middle" justify="center" className="upload-action">
                    <Col>
                      <Space>
                        {uploadFileRender.status !== 'error' && <EyeOutlined onClick={d.preview} />}
                        <DeleteOutlined
                          onClick={() => {
                            d.remove()
                            clearErrors([name])
...
export default FormUploadWithName
