...
export default (t) => (
  yup.object().shape({
    unitSurveyName: yup
      .string()
      .max(200, t('error_message:validation.max_length', { key: t('edit_survey.unit_survey_name'), max: 200 }))
      .required(t('error_message:validation.required', { key: t('edit_survey.unit_survey_name') }))
      .trim(),
    unitSurveyDetail: yup
      .string()
      .max(4000, t('error_message:validation.max_length', { key: t('edit_survey.unit_survey_detail'), max: 4000 }))
      .trim(),
    messageCompleted: yup
      .string()
      .max(100, t('error_message:validation.max_length', { key: t('message_complete'), max: 100 }))
      .trim(),
    listQuestionId: yup.array().min(1, t('error_message:validation.required', { key: t('edit_survey.question_setting') })),
