...
import { formatCSVToUTF8 } from 'APIs'
import { Title, Table } from 'Components'
import { DOWNLOAD_CSV } from 'Constants'
import { downloadOctetFile, mappingFileNameWithRole } from 'Utils'
import { useRegisterByCsv, useGroupAttribute, useRoles, useEnterKeyPress } from 'Hooks'
...
const BatchRegisterUserScreen = () => {
  ...
  const { registeredUsers, registerByCsvAction, resetDataCsv } = useRegisterByCsv()
  const {
    attributes,
    groups,
    loadGroupsAction,
  ...
  const transFormData = async (data, index) => {
    ...
    try {
      ...
      await schema.validateSync({
        ...
        password: dataTransForm.password,
  ...
  const onRegisterUser = () => {
    ...
    registerByCsvAction(dataTransform)
  }
  const onCloseGuiderModal = () => setVisible(false)

  useEnterKeyPress(btnRegisterRef, [dataSource])
  const handleAfterShowDialog = () => {
    ...
    <Spin spinning={isSubmitting} size="large">
      <Wrapper>
        <Title
          ...
          title={t('batch_register_user.title')}
        />
...
export default BatchRegisterUserScreen
