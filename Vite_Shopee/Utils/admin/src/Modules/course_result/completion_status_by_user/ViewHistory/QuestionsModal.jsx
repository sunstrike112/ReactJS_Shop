...
const QuestionsModal = ({ onClose, recordId, typeSelected }) => {
  ...
  const renderSelectOne = (record) => (
    <>
      {record.listAnswer && record.listAnswer.map((answer) => (
        <>
          <Radio style={{ margin: 10, 'white-space': 'pre-line' }} checked={answer.selectedAnswer} disabled>{answer.answerText}</Radio>
          <br />
        </>
      ))}
    </>
  ...
  const renderSelectMulti = (record) => (
    <>
      {record.listAnswer && record.listAnswer.map((answer) => (
        <>
          <Checkbox style={{ margin: 10 }} checked={answer.selectedAnswer} disabled>{answer.answerText}</Checkbox>
          <br />
        </>
      ))}
    </>
  ...
  const renderDescriptionQuestion = (record) => (
    <>
      {record.listAnswer && record.listAnswer.map((answer) => (
        <>
          <TextArea style={{ color: '#323232', 'white-space': 'pre-line' }} rows={4} disabled value={answer.answerTextDescription} />
          <br />
        </>
      ))}
    </>
  ...
  const renderDescriptionSurvey = (record) => (
    <>
      <TextArea style={{ color: '#323232', marginTop: 10, 'white-space': 'pre-line' }} rows={4} disabled value={record.answerText} />
      <br />
    </>
  )
  ...
  const renderDescriptionReport = (record) => {
    if (record.isDraft) {
      return (
        <>
          <TextArea style={{ color: '#323232', marginTop: 10, 'white-space': 'pre-line' }} rows={4} disabled value={record.textDraft} />
          <br />
        </>
      )
    }
    return (
      <>
        <TextArea style={{ color: '#323232', marginTop: 10, 'white-space': 'pre-line' }} rows={4} disabled value={record.answerTextDescription} />
        <br />
      </>
    )
  }
  ...
  const columns = [
    ...
    }),
    {
      ...
      render: (_, record) => (
        <>
          <div style={{ 'white-space': 'pre-line' }} dangerouslySetInnerHTML={{ __html: sanitize(record.questionName) }} />
          {renderAnswer(record)}
        </>
      )
    }
