...
function Index({ pathName, mainUrl, query }) {
  ...
  return (
    <OGBasePageComponent>
      ...
      {resourceName && can && <OGDetailResources resource={resource} idData={query?.id} />}
      {resourceName && !can &&
        <Result
          status="403"
          title="403"
...
Index.getInitialProps = async (ctx) => {
  return { pathName: ctx.asPath, mainUrl: ctx.pathname, query: ctx.query }
}
