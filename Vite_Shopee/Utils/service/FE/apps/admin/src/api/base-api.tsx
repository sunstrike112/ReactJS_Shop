...
import { BASE_API_URL, ORGANIZATIONS_ENDPOINT } from '@ss-fe-fw/constants'
import { notification } from 'antd'
import { APP_LOGIN_URL, RECOIL_KEY, REFRESH_ENDPOINT, DEFAULT_ORGANIZATION_ID } from '@ss-fe-fw/constants'
...
async function baseApi(endpoint, method = 'GET', body = {}) {
  ...
  const xOrganizationId = localStorage.getItem('xOrganizationId') || DEFAULT_ORGANIZATION_ID;
  ...
  try {
    ...
    if (token) {
      ...
      if (endpoint !== (ORGANIZATIONS_ENDPOINT.BASE + ORGANIZATIONS_ENDPOINT.SEARCH)) {
    ...
    if (response?.status == 401 && endpoint !== REFRESH_ENDPOINT) {
      ...
      if (!result.status) {
        ...
        return await baseApi(cachePrevRequest.endpoint, cachePrevRequest.method, cachePrevRequest.body)
      } else {
    ...
    return { message: response?.data?.message ?? 'API System\'s error', status: response?.status ?? 500 }
  }
}
